...
static ssize_t char_device_read(struct file *filep, char *buf, size_t len, loff_t *off) {
  ...
  while (len > 0) {
    ...
    put_user(switches & 0xFF, buf++); //Accessing directly the buffer from user space is a security breach!
    put_user((switches >> 8) & 0xFF, buf++); //Does the put_user function replaces the copy_to/from_user?
                                        // Yes! It does! But the buffer should have been verified with access_ok()
                                        // Maybe he had not did that because the buffer has been generated
                                        // by the ioread16() -> Trustable function?
    len -= 2;
  }
  return count;
}
...
static ssize_t char_device_write(struct file *filep, const char *buf, size_t len, loff_t *off) {
    // buf is from user space!
  char *ptr = (char *) buf; //Does it changes the address? Should the function copy_from_user (or get_user) be used here?
  size_t count = len;
  short b = 0;
...
static struct pci_driver pci_driver = {
  ...
  .id_table = pci_ids,
  .probe    = pci_probe,
  .remove   = pci_remove, //pointer for functions (*suspend) and (*resume) could be inserted too. They are optional
};
...
static int pci_probe(struct pci_dev *dev, const struct pci_device_id *id) {
  ...
  hexport = ioremap_nocache(resource + 0XC000, 0x20); //Adderesses configured on QSys!!!!!
